...
/****** Object:  StoredProcedure [dbo].[usp_ssis_high_level_execution_statistics]    Script Date: 2/2/2018 10:40:35 PM ******/
...
AS
    BEGIN
        ...
        WITH    ProvidedParameters
                  AS (SELECT    IQ.PackageName
                               ,IQ.FolderName
                               ,IQ.ProjectName
                      ...
                      FROM      ( VALUES ( @PackageName, @FolderName, @ProjectName, @WindowBegins, @WindowEnds, @Status) ) IQ (PackageName, FolderName, ProjectName, WindowBegins, WindowEnds, StatusValue)
                     )
            SELECT  E.package_name AS PackageName
                   ...
                   ,SUM(CASE E.status
                          ...
                          ELSE 0
                        END) AS Success
                   ,SUM(CASE WHEN E.status NOT IN (2, 4, 7) THEN 1
                             ELSE 0
                        END) AS Other
            ...
            CROSS APPLY (SELECT CONVERT (DATETIME, E.start_time) AS StartDateTime
                               ...
                               ,CONVERT(FLOAT, DATEDIFF(SECOND, E.start_time, ISNULL(E.end_time, SYSDATETIMEOFFSET()))) / 100 AS Duration
                        ) CALC (StartDateTime, EndDateTime, Duration)
